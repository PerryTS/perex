...
`BoundProgram` and `BoundSubject` validate original storage once and retain its owner plus fixed-size layout metadata. They retain no subject/program slice or movable address. `with_view` obtains a fresh scoped slice from that same owner and performs only length/representation checks, plus the program's seven-word header check. It does not scan the program tables or recount the subject. `BoundResources` combines independent bindings for `Search`, allowing one program binding to serve multiple subject bindings.
