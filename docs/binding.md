...
- `BoundSubject::new_counted(storage, utf16_len)` takes the UTF-16 length the host already holds. It checks only what is constant work: UTF-16 storage must have exactly that many units, and byte storage a length that many units could occupy, one to three bytes each. The representation follows the same rule as validation, ASCII exactly when bytes and units are equal in number. That rule is sound for generalized UTF-8 and only for it: a host counter that charges a truncated multi-byte lead its nominal units reports one unit for the single byte `0xC3`, which is neither ASCII nor well formed, and binding it here would match it as `U+00C3`. A length that did not come from a successful decode belongs in `BoundSubject::new`, which decodes. Perry's own counter has that property, and its regex path stays sound because it marks a string counted only after this crate has decoded it and agreed (Perry #6085, #10166).
