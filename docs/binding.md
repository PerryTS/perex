...
This is the embedding boundary. It does not by itself establish a host's root, sharing or constructor invariants; the host must, as Perry's runtime adapter does. A cached string length or an equal-sized allocation is not a validation certificate.
