...
The synchronous `find` API uses the same evaluator as the experimental [resumable `Search`](resumption.md). An operation can retain partial work and release every input/program view between advances, then restore offsets against fresh validated views from a rooted immutable owner. Explicit replacement transfers live scratch to host-allocated buffers after frame/undo exhaustion, preserving the operation's remaining work. Efficient host view reacquisition remains unfinished. Program/input validation still requires scans, and initial/backreference seeks in byte storage can be linear. These costs and the actual host borrow/root boundary need implementation and measurement before adoption, without copying subjects or hiding an unbounded GC pause.
