...
Perex is an independent ECMAScript regex engine (compiler + evaluator) built for embedders that own their own memory — chiefly the Perry JS runtime. `no_std`, no dependencies, `#![forbid(unsafe_code)]`, unpublished, version `0.0.0`.
...
- `PROBE OUTPUT_DIR [PROBE_ARGS...]`: `check-candidate`, `check-end-candidate`, `check-classes`, `check-unicode-sets`, `check-atom-filter` — CI runs each three times, plain and with `--quantum 1|17 --relocate --grow` (paused execution + simulated collector + scratch growth)
