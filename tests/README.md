...
`fixtures/reference.json` binds fixture/answer hashes and records the Node/V8/Unicode versions used to generate the answers. The comparator rejects missing, duplicate, extra or different answers. The Perex `engine_probe` emits this schema for supported cases and explicit unsupported/error outcomes otherwise. `check-engine.mjs` preserves all cases and compares complete results; its strict mode still fails on the current omissions and reviewed Node disagreement. See `docs/engine.md` for development CI scope.
